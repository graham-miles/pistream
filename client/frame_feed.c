...
unsigned char toret[WIDTH*HEIGHT*3];
...
int get_frame(void)
{
        ...
        image_buffer = (unsigned char *) malloc(WIDTH*HEIGHT*NB_CHAN*4+1024);
    rgb_buffer = (unsigned char *) malloc(WIDTH*HEIGHT*3);
        ...
        for (inc = 0; inc < (nbFrames && retry_counter < 5); )
        {
                ...
                nb = 0;
                retry_pixel = 0;
                while (nb < ((WIDTH)*(HEIGHT)*NB_CHAN)*3 && retry_pixel < 10000)
                {
                        wishbone_read((unsigned char *) cmd_buffer, 6, FIFO_ADDR+FIFO_CMD_OFFSET);
            ...
            fclose(jpeg_fd);
                ...
                end_ptr = &image_buffer[WIDTH*HEIGHT*NB_CHAN*3];
                ...
                memcpy(toret, rgb_buffer, WIDTH*HEIGHT*3);
                write_jpegfile(rgb_buffer, WIDTH, HEIGHT, 3, jpeg_fd, 100);
                fclose(jpeg_fd);
        }
